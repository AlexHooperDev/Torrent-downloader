...
export async function getTorrentOptions(title: string, year?: string): Promise<TorrentOption[]> {
  ...
  const res = await fetch(`http://localhost:3000/torrents?${params.toString()}`);
...
export async function getTrending(media: "movie" | "tv" | "all" = "all", pages = 3): Promise<CatalogItem[]> {
  ...
  const res = await fetch(`http://localhost:3000/trending?${params.toString()}`);
  if (!res.ok) throw new Error("Failed to fetch trending");
  return res.json();
}

export async function getNewTv(): Promise<CatalogItem[]> {
  const res = await fetch("http://localhost:3000/tv/new");
...
export async function getShowDetails(tvId: number): Promise<ShowDetailsApi> {
  const res = await fetch(`http://localhost:3000/tv/${tvId}/details`);
  if (!res.ok) throw new Error("Failed to fetch show details");
  return res.json();
}

export async function getSeasonEpisodes(tvId: number, season: number): Promise<EpisodeInfo[]> {
  const res = await fetch(`http://localhost:3000/tv/${tvId}/season/${season}`);
...
export async function getGenres(media: "movie" | "tv" = "movie"): Promise<GenreItem[]> {
  ...
  const res = await fetch(`http://localhost:3000/genres?${params.toString()}`);
...
export async function discoverCatalog({
  ...
  const res = await fetch(`http://localhost:3000/discover?${params.toString()}`);
  if (!res.ok) throw new Error("Failed to discover");
  return res.json();
}

export async function purgeTorrents(): Promise<void> {
  await fetch("http://localhost:3000/torrents/purge", { method: "POST" });
}
...
export async function getTorrentProgress(magnet: string): Promise<TorrentProgress | null> {
  ...
  const res = await fetch(`http://localhost:3000/torrent/progress?${params.toString()}`);
...
export async function getMovieDetails(movieId: number): Promise<MovieDetailsApi> {
  const res = await fetch(`http://localhost:3000/movie/${movieId}/details`);
...
export async function getMovieVideos(movieId: number): Promise<VideoItem[]> {
  const res = await fetch(`http://localhost:3000/movie/${movieId}/videos`);
  if (!res.ok) return [];
  return res.json();
}

export async function getShowVideos(tvId: number): Promise<VideoItem[]> {
  const res = await fetch(`http://localhost:3000/tv/${tvId}/videos`);
  if (!res.ok) return [];
  return res.json();
} 